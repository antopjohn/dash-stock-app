...
server = app.server  # For Gunicorn / Render / AWS deployment
...
app.layout = dbc.Container([
    ...
    dcc.Store(id='user-watchlist-store', storage_type='session'),  # per-user watchlist

    dcc.Tabs([
        dcc.Tab(label="Dashboard", children=[
            html.Br(),
            html.H5("Add / Remove Symbols"),
            dcc.Input(id='symbol-input', placeholder='Enter symbols comma separated', debounce=True, style={'width':'50%'}),
            html.Br(), html.Br(),
            dbc.Button("Add Symbol(s)", id="add-symbol-btn", color="primary", className="me-2"),
            dbc.Button("Remove Selected", id="remove-symbol-btn", color="danger"),
            html.Br(), html.Br(),
...
# ------------------------------
# Multi-user Watchlist Callback
# ------------------------------
@app.callback(
    Output('symbol-input','value'),
    Output('watchlist-table','data'),
    Output('watchlist-table','style_data_conditional'),
    Output('user-watchlist-store','data'),  # now added
    Input('add-symbol-btn','n_clicks'),
    Input('remove-symbol-btn','n_clicks'),
    State('symbol-input','value'),
    State('watchlist-table','data'),
    State('watchlist-table','selected_rows'),
    State('user-watchlist-store','data'),
    prevent_initial_call=False
)
def modify_watchlist(add_clicks, remove_clicks, input_symbols, table_data, selected_rows, stored_symbols):
    symbols = stored_symbols if stored_symbols else []

    trigger = callback_context.triggered[0]['prop_id'].split('.')[0]

    if trigger == 'add-symbol-btn' and input_symbols:
        new = [s.strip().upper() for s in input_symbols.split(',') if s.strip()]
        symbols = list(dict.fromkeys(symbols + new))

    if trigger == 'remove-symbol-btn' and selected_rows:
        remove = [table_data[i]['Symbol'] for i in selected_rows]
        symbols = [s for s in symbols if s not in remove]

    if symbols:
        df_watchlist = compute_signals(symbols)
    ...
    return "", data, style_conditional, symbols
