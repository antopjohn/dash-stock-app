...
# Batch Signals
# ------------------------------
def compute_signals_batch(symbols):
    if not symbols:
        return pd.DataFrame(columns=['Symbol','Price','9MA','45MA','200MA','Signal','Signal_Icon'])
    ...
    for symbol in symbols:
        try:
            if len(symbols) == 1:
                hist = flatten_yf_data(data)
            else:
                if symbol not in data.columns.levels[0]:
                    continue
                hist = flatten_yf_data(data[symbol])
...
app.layout = dbc.Container([
    ...
    dcc.Interval(id='interval-update', interval=5*1000, n_intervals=0),  # 5s refresh
    dcc.Tabs([
